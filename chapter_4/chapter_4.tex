...
\cite{nadafiRobustBacksteppingAttitude2021} implement a controller based on backstepping. Backstepping is a recursive design method for nonlinear systems \cite{khalilNonlinearSystems2002}.
...
\end{figure}
\newpage
\section{Nadafi and KabganianController}
...
\section{Zarourati/Tian Controller}

It can be  \cite{nadafiRobustBacksteppingAttitude2021} \cite{zarouratiBacksteppingSlidingMode2022}a backstepping sliding-mode adaptive controller is designed to control an underactuated spacecraft and achieve pointing. The controller is robust to external disturbances, varying inertia disturbances, reaction wheel torque disturbances and is fast to run in contrast to a control method such as model predictive control. This latter benefit is highly valuable on limited compute resources such as a satellite.

\begin{align}
u_c^r &= C_r^{T}\Big[\underbrace{S_r(\omega)H_r - J_rS_r(\omega_e)A_r\omega_{d,r}+J_rA_r\dot\omega_{d,r}}_{\text{known/feedforward} \sim \mathbf F}
