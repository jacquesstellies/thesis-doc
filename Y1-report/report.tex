...
Typically for FTC systems two design approaches are available: passive and active. Passive FTC is composed of fixed controller which is designed to be robust to a set of predefined faults. Typically it is more conservative as it trades nominal performance for robustness. It also utilizes redundant actuators. It does not require any online fault information and does not require controller reconfiguration. 
\par Active FTC, on the other hand, is designed to reconfigure the controller in the presence of a fault. It can achieve this through preselecting an existing control law or synthesizing a new control law. It therefore requires online fault detection and diagnosis (FDD) information. Typically a decision mechanism is used to distinguish between the faulty and non-faulty cases of operation usually via some type of threshold. If well designed this should compensate for some model uncertainty and noise to reduce false positives. Degraded performance of the system does have to be accepted when entering fault state but using this strategy one avoids increasing the conservativeness of the nominal controller \cite{yinReviewRecentDevelopment2016}.
...
Fault detection and diagnosis (FDD) is crucial for providing up to date fault information for controller reconfiguration. At a high level FDD can be summarized by asking the following questions:
\begin{itemize}
...
Approaches to FDD can be broken down into 3 broad categories: model-based, data-driven and hybrid. 
The first step is to detect a fault. To minimize the risk of false detections due to model uncertainties and to ensure robustness to noise a detection threshold is commonly used. In cite{}
...
\subsection{Fault Detection and Diagnosis}
For the purposes of this paper the fault will be assumed to be perfectly known and injected directly into the system and the FTC unit will be used to reconfigure the controller. Mathematically the estimated fault can be incorporated into the control law as follows:
