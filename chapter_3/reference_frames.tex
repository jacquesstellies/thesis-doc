...
\triad{Ecen}{1.2}{I}
\node[anchor=north east,xshift=-2pt,yshift=-13pt] at (Ecen) {\small $F_I$};

% vernal equinox direction: continuation of the x_I axis, starting beyond
% the Earth's edge so it doesn't overlap the triad arrow
\draw[-{Latex[length=2.2mm]}, black, thick] ($(Ecen)+(210:4)$) -- ++(210:0.85) node[above right=-20pt] {\small $\aries$};
...
\node[orbitorange!70!black] at ($(Ecen)+(-4.1,-1.05)$) {\small Orbit};
...
\node[sloped] at ($(Ecen)+(1.0,1.4)$) {\small $\vec{r}_{BE}$}; %{\small $[\bm{R}]^{OI},\ [\bm{t}]^{OI}$}
...
\triad{Sat}{1.05}{B}
\end{scope}
\node[anchor=north west,xshift=0pt,yshift=-3pt] at (Sat) {\small $F_B$};
\end{tikzpicture}
